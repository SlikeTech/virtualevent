var Chat = (function(){
        ...
        $(".chat-floater .minimise-icon").off("click").on("click", function(){
        	if( $(".chat-cOuter").hasClass("H")){
        		...
        		$(".chat-floater").addClass("show-minimise-chat")
        		 ...
        		 $(".chat-floater").removeClass("hide-minimise-chat")
        	}
        })
    ...
    var getAllUsersAndGroups = function(){
        // var rms = EventStore.getStore().eventData.chats;
        // var actPage = EventStore.getActivePage();
        // var users = EventStore.getAttendies();

        // lhsActiveArr_ = ;
        // rms.filter(o => {
        //     lhsActiveArr_.push({name:o.room_name, uuid:o.uuid, type:"room"})
        // })

        // users.filter(o => {
        //     lhsActiveArr_.push({name:o.firstname+" "+o.lastname, uuid:o.uuid, type:"user"})
        // })

        lhsActiveArr_ = chats_.list.rooms.concat(chats_.list.users)

        $(".full-chat .chat-user").html(generateHTML());
    ...
    var generateHTML = function(){
        var str = ""
        // for(var i=0; i<lhsActiveArr_.length; i++){
        //     str += '<div class="chat-row">'
        //     str += '<i class="material-icons">account_circle</i>'
        //     str += '<div class="chat-name text-truncate">'+lhsActiveArr_[i].name+'</div>'
        ...
        var h, l;
        for(var i=0; i<lhsActiveArr_.length; i++){
            str += '<div class="chat-row" data-id="'+lhsActiveArr_[i]+'">'
            str += '<i class="material-icons">account_circle</i>'
            str += '<div class="chat-name text-truncate">'+ chats_.master[lhsActiveArr_[i]].name+'</div>'
            
            
            //Unread chat
            console.log(lhsActiveArr_[i])
            //debugger
            l = chats_.messages[lhsActiveArr_[i]].newMessage.length
            h = l === 0 ? "H" : "l"
                      ...
                      for(i=0; i< r.length; i++){
                          if(!chats_.master[r[i].ID]){
                              chats_.master[r[i].ID] = {
                                   name: r[i].ID === eventId ? "EVENT" : r[i].Title,
                                   type: "room",
                                   page:r[i].ID,
 ...
 return '<div class="full-chat">'+
        ...
        '                <div class="chat-header">'+
        '                </div>'+
        '                <div class="chat-messages H" style="height: calc(100% - 140px);">'+
        '                </div>'+
...
'                        </div>'+
'                        <div class="dock-Icon">                            '+
'                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="14" viewBox="0 0 21 14"><defs><style>.a{fill:#ababab;}.b{fill:none;stroke:#ababab;}</style></defs><g transform="translate(-889.5 -104)"><rect class="a" width="7" height="14" transform="translate(903.5 104)"/><line class="b" x1="20" transform="translate(890 104.5)"/><path class="b" d="M20,0H0" transform="translate(890 117.5)"/><line class="b" y1="13" transform="translate(890 104.5)"/></g></svg>'+
'                        </div>'+
'                        <div class="close-Icon">                            '+
'                            <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11.002" viewBox="0 0 11 11.002"><defs><style>.a{fill:#ababab;}</style></defs><path class="a" d="M18.089,16.79l3.929-3.931a.921.921,0,1,0-1.3-1.3l-3.929,3.931-3.929-3.931a.921.921,0,1,0-1.3,1.3l3.929,3.931L11.556,20.72a.921.921,0,0,0,1.3,1.3l3.929-3.931,3.929,3.931a.921.921,0,1,0,1.3-1.3Z" transform="translate(-11.285 -11.289)"/></svg>'+
'                        </div>'+
'                    </div>'+
'                    <div class="chat-header">'+
'                        <i class="material-icons">account_circle</i>'+
...
var genFullChat123456 = function(){
        return '<div class="full-chat">'+
        '                <div class="fChat-lhs">'+
        '                <div class="fChat-header">'+
        '                    <div class="booth-avatar">'+
...
/*
        '                        <div class="chat-row">'+
        '                            <i class="material-icons">account_circle</i>'+
        '                            <div class="chat-name text-truncate">Day 1 - Audi Common Chat Room</div>'+
        '                            <div class="chat-date H">'+
        '                                10:30 AM '+
        '                                <span>'+
        '                                    12'+
        '                                </span>'+
        '                            </div>'+
        '                            <div class="add-user">'+                             
        '                                <svg xmlns="http://www.w3.org/2000/svg" width="30.013" height="18.013" viewBox="0 0 30.013 18.013"><defs><style>.a{fill:#138329;}</style></defs><path d="M16.881,13.506a4.5,4.5,0,1,0-4.5-4.5A4.516,4.516,0,0,0,16.881,13.506Zm0,2.252c-2.983,0-9.006,1.52-9.006,4.5v2.252H25.888V20.261C25.888,17.278,19.865,15.758,16.881,15.758Z" transform="translate(4.125 -4.5)"/><path d="M9.218,3.375a5.843,5.843,0,1,0,5.843,5.843A5.843,5.843,0,0,0,9.218,3.375Zm3.006,6.433H9.808v2.416H8.628V9.808H6.212V8.628H8.628V6.212h1.18V8.628h2.416Z" transform="translate(-3.375 -3.375)"/></svg>'+
        '                            </div>'+
        '                        </div>'+
        ...
        */
        ...
        '                <div class="chat-header">'+
        /*'                    <div class="dF">'+
        '                    <i class="material-icons f45">group_add</i>'+
        '                    <div class="chat-group">'+
        '                        <b>CREATE GROUP</b>'+
        '                        <span>Create your custom group and add participants</span>'+
        '                    </div>'+
        '                    </div>'+*/
        '                </div>'+
       ...
       /* '                    <div class="group-name">'+
        '                        <input type="text" id="groupName" name="" placeholder="Your group name here">'+
        '                    </div>'+
        '                    <div class="user-add">'+
        '                        <div class="user-add-header">Users added in group</div>'+
        '                        <div class="user-add-list">'+
        '                            <div class="chip"><span class="text-truncate">Tag 220</span> <i class="material-icons">cancel</i></div>'+
        '                            <div class="chip"><span class="text-truncate">Tag 220</span> <i class="material-icons">cancel</i></div>'+
        '                            <div class="chip"><span class="text-truncate">Tag 220</span> <i class="material-icons">cancel</i></div>'+
        '                        </div>'+
        '                        <div class="user-add-btn>'+
        '                            <button class="lightBtn" type="button">Cancel</button>'+
        '                           <button class="lightBtn" type="button">Save</button>'+
        '                        </div>'+
        '                    </div>'+*/
        '                </div>'+
        '                </div>'+
      ...
      /*  '                    <div class="dF">'+
        '                        <i class="material-icons f45">account_circle</i>'+
        '                        <div class="chat-group">'+
        '                            <b>Networking Zone</b>'+
        '                            <span>Members: 226</span>'+
        '                        </div>'+
        ...
        '                        <!-- Chat Group -->'+
        '                        <div class="exit-group H">'+
        '                            EXIT GROUP'+svgs("exit")+
        '                        </div>'+

        
        '                        <!-- Single User -->'+
        '                        <div class="user-icons">'+
        '                            <div class="user-action"   data-id="msg-vid">'+svgs("video")+
        '                            </div>'+
        '                            <div class="user-action" data-id="msg-email">'+svgs("email")+
        '                            </div>'+
        '                            <div class="user-action" data-id="msg-bcard">'+svgs("bcard")+
        '                            </div>'+
        '                            <div class="user-action"  data-id="msg-usr">'+svgs("user")+
        '                            </div>'+
        '                        </div>'+
        '                    </div>'+*/
       ...
       /* '                    <div class="row admin-chat">'+
        ...
        '                            <svg xmlns="http://www.w3.org/2000/svg" width="19.54" height="13.19" viewBox="0 0 19.54 13.19"><defs><style>.a{fill:#acacac;}</style></defs><path class="a" d="M.072,16.09V5.236q0-.019.057-.358l6.388,5.465L.147,16.467a1.6,1.6,0,0,1-.075-.377ZM.92,4.125a.812.812,0,0,1,.32-.057h17.2a1.067,1.067,0,0,1,.339.057L12.377,9.608l-.848.678L9.852,11.662,8.175,10.286l-.848-.678ZM.939,17.2,7.364,11.04l2.487,2.016,2.487-2.016L18.764,17.2a.9.9,0,0,1-.32.057H1.24a.853.853,0,0,1-.3-.057Zm12.248-6.859,6.369-5.465a1.125,1.125,0,0,1,.057.358V16.09a1.444,1.444,0,0,1-.057.377Z" transform="translate(-0.072 -4.068)"/></svg>'+
        '                        </div>'+
        '                        <div class="business-icon">'+
        '                            <svg xmlns="http://www.w3.org/2000/svg" width="17.967" height="13.974" viewBox="0 0 17.967 13.974"><defs><style>.a{fill:#acacac;}</style></defs><path class="a" d="M16.47,2.25H1.5A1.5,1.5,0,0,0,0,3.747v10.98a1.5,1.5,0,0,0,1.5,1.5H16.47a1.5,1.5,0,0,0,1.5-1.5V3.747A1.5,1.5,0,0,0,16.47,2.25ZM5.49,5.244a2,2,0,1,1-2,2A2,2,0,0,1,5.49,5.244Zm3.494,7.386a.654.654,0,0,1-.7.6H2.7a.654.654,0,0,1-.7-.6v-.6a1.964,1.964,0,0,1,2.1-1.8h.156a3.213,3.213,0,0,0,2.483,0h.156a1.964,1.964,0,0,1,2.1,1.8Zm6.987-1.647a.25.25,0,0,1-.25.25H11.229a.25.25,0,0,1-.25-.25v-.5a.25.25,0,0,1,.25-.25h4.492a.25.25,0,0,1,.25.25Zm0-2a.25.25,0,0,1-.25.25H11.229a.25.25,0,0,1-.25-.25v-.5a.25.25,0,0,1,.25-.25h4.492a.25.25,0,0,1,.25.25Zm0-2a.25.25,0,0,1-.25.25H11.229a.25.25,0,0,1-.25-.25v-.5a.25.25,0,0,1,.25-.25h4.492a.25.25,0,0,1,.25.25Z" transform="translate(0 -2.25)"/></svg>'+
        '                        </div>'+
        '                    </div>'+*/
        '                </div>'+
        '            <div class="chat-input">'+
